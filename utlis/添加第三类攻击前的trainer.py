...
def train_step(args, epoch, batch, model, alice_bob_mac, key_ab, eve, Alice_KB, Bob_KB, Eve_KB, Alice_mapping, Bob_mapping, Eve_mapping, bleu_predictor, src, trg, src_eve, n_var, pad, opt_joint, channel, mi_net=None):  # 模型，发送的128个句子，发送的128个句子，噪声标准差(类型数字)，数字0，deepsc优化器，信道类型
    ...
    src_eve = src_clone[indices]  # 这得改一下 换成传进来的参数
    ...
    # 在这写添加了KB之后的一个最简单的DeepSC 不包含mac的训练
    freeze_net(key_ab, False)
    freeze_net(alice_bob_mac, False)
    ...
    # 下面开始是原来的 为了测试一下能跑通吗就先注释掉了
    # 现在加回来了

    # # 下面是训练对key的敏感程度 准备放弃了
    # freeze_net(key_ab, True)
    # freeze_net(alice_bob_mac.mac_encoder, True)
    # freeze_net(alice_bob_mac.mac_decoder, True)  # 得训练 因为要提高对key的敏感程度
    # freeze_net(eve.mac_encoder, False)
    # freeze_net(eve.noise, False)
    ...
    # enc_output = model.encoder(src, src_mask)  # f   明天问问这个掩码怎么办
    ...
    # # 全是0
    # targets_key = torch.zeros(bs, 1).to(device)
    # targets_key = targets_key * (1 - smoothing) + (1 - targets_key) * (smoothing / 2)
    # result_key = alice_bob_mac.mac_decoder(mac_p_wrong, f_p, key_ebd)  # 这里用的就是Bob对的密钥了
    # key_0 = criterion_mac(result_key, targets_key)
    # opt_joint.zero_grad()
    # key_0.backward()
    # opt_joint.step()

    # 这里训练一下bleu预测网络（我觉得写错了 应该写一个类似于performance函数的那个）————没写错 但是和greedy的重复度较高 我觉得可以把greed和performance拆了放这里
    for i in range(2):
        freeze_net(key_ab, False)
        freeze_net(alice_bob_mac, False)
        freeze_net(eve, False)
    ...
    #
    #
    # elif batch_mod == 100000:  # 这个就不训了
    #     # eve角度 经过bob的验证1 训练eve
    #     if epoch < 100:
    #         freeze_net(eve.mac_encoder, False)
    #     else:
    #         # freeze_net(eve.mac_encoder, True)
    ...
    #
    #     opt_joint.zero_grad()
    #     (eve_mac_1 + loss_normal).backward()
    #     opt_joint.step()
    ...
    return loss_deepsc.item(), loss_normal.item(), loss_bleu.item(), eve_mac_0.item(), eve_mac_1.item(), tamper_0.item(), tamper_1.item()


def val_step(args, batch, model, alice_bob_mac, key_ab, eve, Alice_KB, Bob_KB, Eve_KB, Alice_mapping, Bob_mapping, Eve_mapping, bleu_predictor, src, trg, src_eve, n_var, pad, channel):  # 参数模型，发送的128个句子，发送的128个句子，噪声标准差(数字0.1)，数字0，信道类型
    ...
    return loss_deepsc_test.item(), loss_normal_test.item(), loss_bleu_test.item(), eve_mac_0_test.item(), eve_mac_1_test.item(), tamper_0_test.item(), tamper_1_test.item()
...
def greedy_decode(args, deepsc, alice_bob_mac, key_ab, eve, Alice_KB, Bob_KB, Eve_KB, Alice_mapping, Bob_mapping, Eve_mapping, src, src_eve, noise_std, max_len, padding_idx, start_symbol, channel):
    ...
    return outputs, zheng_mac_accuracy, eve_mac_0_accuracy, tamper_0_accuracy
