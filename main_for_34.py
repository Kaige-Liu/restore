...
from utlis.trainer_for_34 import initNetParams, train_step, val_step, train_mi
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(r'C:\d\final\restore\checkpoints\checkpoint_109.pth')  # 之前三个检测率都最好
    ...
    for epoch in range(args.epochs):  # 默认训练完整的数据集80轮
        ...
        if loss_eps_test < record_loss:  # 如果验证的loss小于之前的loss（性能更好了）
            ...
            torch.save(checkpoint, './checkpoints/12/' + now + '/checkpoint_{}'.format(epoch) + '_{}'.format(str(loss_eps_test)[:6]) + '_{}.pth'.format(str(eve_0)[:6]))  # 保存模型
