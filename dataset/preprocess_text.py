...
def main(args):
    data_dir = 'E:\\WorkSpace_Python\\deepsc-torch\\data\\'
    ...
    test_data = results[round(len(results) * 0.9):]  # 这里是取90%的数据作为训练数据，剩下的10%作为测试数据

    with open(args.output_train_dir, 'wb') as f:
        pickle.dump(train_data, f)
    with open(args.output_test_dir, 'wb') as f:
        pickle.dump(test_data, f)  # dump的作用是将数据序列化后保存到文件中
