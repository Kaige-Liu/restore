...
class Mine(nn.Module):
    def __init__(self, in_dim=2, hidden_size=10):  # 输入维度为2，隐藏层维度为10
        super(Mine, self).__init__()

        self.dense1 = linear(in_dim, hidden_size)  # 其实就是定义了三个层
        self.dense2 = linear(hidden_size, hidden_size)
        self.dense3 = linear(hidden_size, 1)

    def forward(self, inputs):  # 前向传播，其实就是经历了上面的三个层（还有添加了激活函数RELU）
